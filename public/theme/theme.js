...
var datatableDatos = {
  ...
  config: function(){
    this.body = body;
    this.idioma = $('#inpt_Idioma').val();
    return;
  },
  comportamiento: function(){

    var table = this.body.find('#table');
    ...
    $.extend(opciones, opcionesLang);

    table.DataTable(opciones);
...
var alineacionRelaciones = [];
var alineacionRelacionesJson ="";
  
var asignarAtributosAsistente = {
  ...
  config: function(){
    ...
    this.fieldset = this.col02.find('.fieldsetContent');
    var divalineacionEntidades = this.col02.find('#alineacionEntidades');
    divalineacionEntidades.prepend('<div class="atributosAsignados"></div>');
    
    return;
  },
  comportamiento: function(){

    var btnAsignar = this.fieldset.find('.btnAsignar');
    var divAsignados = this.fieldset.find('.atributosAsignados');
 
    //Asignamos el valor al div
    btnAsignar.off('click').on('click', function(){
      var that = $(this);
      if (ponerAsignadoJson(that)) {
      
         if(divAsignados.find('.form-group').length == 0) divAsignados.show();

         var divCampo = that.parents('.form-group');
         var valorLabel = divCampo.find('> label').text();
         var idSelect = divCampo.find('select').attr('id');
         var textoSelect = divCampo.find('#' + idSelect + ' option:selected').text();
         divCampo.hide();

        //Creamos nuevo div
        var campo = $('<div />').addClass('form-group form-atributo');
        var label = $('<label />').text(valorLabel);
        var idAsignado = idSelect + '_asignado';
        var input = $('<input type="text" id="' + idAsignado + '" class="text medium" value="' + textoSelect + '" disabled>');
        var boton = $('<button type="button">Quitar Atributo</button>').addClass('btnEliminar');
        campo.append(label).append(input).append(boton);

        divAsignados.append(campo);
      }
        
    });

    //Eliminamos atributo
    this.fieldset.off('click').on('click', '.btnEliminar', function(){
      var that = $(this);
      quitarAsignadoJson(that);
      var divCampo = that.parents('.form-group');
      divCampo.remove();

      if(divAsignados.find('.form-group').length == 0) divAsignados.hide();

      //Devolvemos valor
      var inputId = divCampo.find('input').attr('id');
      var id = inputId.replace('_asignado', '');
      var fieldsetContent = divAsignados.parents('.fieldsetContent');
      var select = fieldsetContent.find('#' + id);
      var divForm = select.parents('.form-group');
      divForm.show();
    });
...
function ponerAsignadoJson(that){
    ...
    var valor = that.parent().parent().find("select").val();
    if (valor!=null){
      valor = valor.trim();
    }
    ...
    if (tieneValor) {
       ...
       var elementoBorrar;
       $.each(alineacionRelaciones, function(key, value) {
  	 if (value.campo == campo) {
	    encontrado = true;
	    elementoBorrar = campo;
	 }
       });
       if (encontrado) {
	 alineacionRelaciones = $.grep(alineacionRelaciones, function(value) {
	     return value.campo != elementoBorrar;
	 });
       }
       alineacionRelaciones.push({campo: campo, valor: valor});
       var alineacionRelacionesTempJson = "";
       $.each(alineacionRelaciones, function(key, value) {
  	alineacionRelacionesTempJson = alineacionRelacionesTempJson + '"' + value.campo +'":"' + value.valor + '",';
       });
       alineacionRelacionesJson = "{" + alineacionRelacionesTempJson + "}"
       //alert(alineacionRelacionesJson);
       $('input[name ="alineacionRelaciones"]').val(alineacionRelacionesJson);
    } 
    return tieneValor;         
}

function quitarAsignadoJson(that){

    var valor = that.parent().parent().find("input").val().trim(); 
    var campo = that.parent().find("label").text().trim();
    campo = campo.replace("Alineacion","");
    var encontrado = false;
    var elementoBorrar;
     $.each(alineacionRelaciones, function(key, value) {
...
$(function () {
  ...
  } else if (body.hasClass('asistente')){
    asignarAtributosAsistente.init();
  }
